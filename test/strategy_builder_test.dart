...
void main() {
  testWidgets('Strategy Builder: create, save, duplicate and version history', (WidgetTester tester) async {
    ...
    expect(find.textContaining('ENTER WHEN: RSI(14) > 60 AND Price > VWAP AND Volume > 1.5 × Average Volume AND Market Regime = Bullish'), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.text('Strategy Builder — Momentum Breakout Prototype'), findsOneWidget);

    await tester.tap(find.text('Version History'));
    await tester.pumpAndSettle();
    expect(find.text('Version History — Momentum Breakout Prototype'), findsOneWidget);
  ...
  test('State layer: save, update, duplicate and version tracking', () {
    ...
    final updated = state.strategies[1].copyWith(status: 'BACKTESTING', version: 'v1.4.3');
    ...
    state.saveNewStrategy(updated.copyWith(id: 'STR-999', name: 'Brand New', version: 'v1.0'));
